...
void main() {
  group('FilterViewModel 实时预览', () {
    ...
    test('setTransform 触发 processImage 重跑（预览只跑滤镜）', () async {
      final notifier = container.read(filterViewModelProvider.notifier);
      notifier.setCropRatio(CropRatio.ratio_1_1);
      ...
      // setTransform 仍触发 _runProcess（重跑滤镜）
    ...
    test('_runProcess 不调用 applyTransform (预览只跑滤镜)', () async {
      final notifier = container.read(filterViewModelProvider.notifier);
