...
### Fixed
- **点击取景框对焦位置错位**（〇十七，两轮修复）。`onTapUp` 之前把 frame 归一化点击坐标直接喂给 `setFocusPoint` / `setExposurePoint`，但 sensor 与 frame 比例不一致时 `FittedBox(BoxFit.cover)` 会裁掉一部分（iPhone 16 Pro + 16:9 sensor 上下各裁 12.5%），焦点偏到边缘。修正：新增 `CameraService.mapTapToSensorFocusPoint()` 纯函数反推 cover 裁切找 SizedBox（texture）归一化坐标。
  - **第一轮（commit 3a934e2）**：多做了一步 sensor 旋转反变换（portrait 90° CCW `(x,y)→(1-y,x)`、landscape 180°）。**这一轮走过头了**——iOS `camera_avfoundation` plugin 内部 `cgPoint(for:withOrientation:)` 已经按 `UIDevice.current.orientation` 做了同样的旋转（`/Users/partical/.pub-cache/hosted/pub.dev/camera_avfoundation-0.9.23+2/.../DefaultCamera.swift:939`）。结果：双重旋转，portrait 下焦点从用户点击位置往右下偏，Y 偏差比 X 大。
  - **第二轮（本 commit）**：删掉函数里多余的旋转，只保留 cover 反推。函数现在返回 SizedBox（= texture）归一化坐标，可直接喂 `setFocusPoint`，plugin 内部会自己转回 sensor。portrait + 16:9 sensor 验证：点 (0, 0) → texture (0, 0.125)、点 (1, 1) → texture (1, 0.875)、点 (0.5, 0) → texture (0.5, 0.125)、点 (0.5, 0.5) → texture (0.5, 0.5)。8 个测试（7 个新 + 1 个全链路回归）覆盖 portrait/landscape × 16:9/4:3 sensor × 中心/角落/边界 + iOS plugin cgPoint 复合变换，全量 146 测试通过。
