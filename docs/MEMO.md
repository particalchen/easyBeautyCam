...
**最新**：〇十一 相机 AppBar 布局 + Vision EXIF orientation bugfix（详见 [〇十一](#appbar-and-vision-exif-bugfix-2026-06-24)）

**上一节**：〇十 iOS 端改用 Apple Vision framework（修 iOS sim arm64 不兼容，详见 [〇十](#vision-framework-replacement-2026-06-23)）
...
编辑页磨皮 / 美白只作用于照片中检测到的人脸区域，背景与眼睛 / 嘴唇保留细节（6 commits: `b082e5e` `0693b2c` `2bc99b4` `c09fbf8` `ae0b540` `cfc851c`）：

1. **ML Kit 静态图检测**：`google_mlkit_face_detection ^0.13.2` + `google_mlkit_commons ^0.11.0`；性能模式 `FaceDetectorMode.fast`，`enableContours: true`（拿整脸 / 眼 / 唇轮廓点集），`enableLandmarks/Classification/Tracking` 关；`FaceDetectionService` 按 `imagePath` 缓存（`Map<String, List<FaceContours>>`），避免滑杆拖动时反复 detect；`dispose()` 释放 ML Kit detector；底层抛异常向上抛，ViewModel 决定降级（mask=null=skip beauty）

2. **`FaceContours` 数据类**：纯 Offset 列表（不依赖 ML Kit 的 `Face` 类），含 `face` / `leftEye` / `rightEye` / `lipUpper` / `lipLower` 五个 polygon 字段；`FaceDetectFn = Future<List<FaceContours>> Function(String, Uint8List?)` typedef 注入便于测试

3. **`FaceMaskBuilder` 轮廓 → 1-channel mask**：
   - 全 0 mask 起步 → `img.fillPolygon` 整脸 = 255 → `img.fillPolygon` 眼唇 = 0 排除 → `img.gaussianBlur(mask, radius: 8)` 羽化
   - 纯函数，不依赖 ML Kit，便于单测
   - 5 个单测覆盖：单脸覆盖 / 眼唇排除 / 空 faces / 多脸覆盖 / feather vs sharp 对比

4. **`ImageProcessingService.applyBeauty` 加 `img.Image? mask` 参数**：
   - `mask == null` → 直接 return 原图（per spec 4.1.3：未检测到人脸时无美颜回退，UI 提示「未检测到人脸」）
   - `mask` 提供 → 只在 `mask.r > 0` 的像素做 blend（磨皮）和 `+adjust`（美白），`blendFactor / adjust` 按 mask 灰度衰减（边缘羽化）
   - 顺带修复 3 个 pre-existing bug：① `orig` 从空白 result Image 读（现改成 clone `origImage = img.Image.from(image)`）② `gaussianBlur` 原地污染输入（clone 后再 blur）③ `smooth=0` 分支没把 orig 复制到 result（显式全图 copy loop）
   - 4 个新单测覆盖：mask=null / allZero / fullWhite / half 四种行为

5. **`FilterViewModel` 串接**：注入 `FaceDetectionService` + `FaceMaskBuilder`；`_runProcess` 流水线改为 `applyFilter → detect(缓存命中) → buildMask → applyBeauty(with mask) → normalizeBrightness`；`FilterViewModelState` 新增 `faceCount` (int) / `faceDetectionFailed` (bool)；`setImage` 触发 `_faceDetector.clearCache()` 释放内存；try/catch 包 detect 异常 → 状态置 `faceDetectionFailed=true` → UI 走 warning 分支；`saveProcessedImage` 通过 `_buildMaskForCurrentState()` 复用缓存

6. **UI 反馈**：`BeautySlider` 顶部加 `_buildFaceHint(l10n, state)` 行：
   - `faceDetectionFailed || faceCount == 0` → 橙色 ⚠️ `Icons.warning_amber_rounded` + 「未检测到人脸，美颜未生效」
   - else → 绿色 ✓ `Icons.check_circle` + 「已检测 N 张人脸」（placeholder `count: int`）
   - l10n 加 `beautyNoFaceDetected` / `beautyFaceDetected` 两个 key（zh + en）
   - `AppColors` 新增 `warning = 0xFFFF9800` / `success = 0xFF4CAF50`

7. **范围严格控制**：仅在编辑页静态图上做；**不**做实时视频美颜（spec 排除项）；**不**做瘦脸（slim 保留 stub 参数但不实现）

8. **测试**：62 → 132 个测试，新增高斯 mask 测 × 4、applyBeauty mask 行为 × 4、FilterViewModel 串接 × 4、BeautySlider hint × 3（0/2/失败） = 15 个新测试

---

<a id="vision-framework-replacement-2026-06-23"></a>
### 〇十 iOS 端改用 Apple Vision framework (2026-06-23)

〇九 用 ML Kit 做 iOS 端人脸检测，跑真机没问题但 **iOS 模拟器（Apple Silicon + iOS 26+）跑不起来**——Google ML Kit 的预编译 framework 不发 arm64 simulator 切片（多年公开问题）。本节把 iOS 端换成 Apple 原生 Vision framework，模拟器 / 真机统一走同一条路（5 commits: `bebc3de` `4c2cd81` `d61309a` `f66bb41` `d68cdf5` `a73583b`）。

### 改动

1. **`ios/Runner/FaceDetectionPlugin.swift`**（新文件，~92 行）
   - `FlutterPlugin` 实现，MethodChannel `easy_beauty_cam/face_detection`，方法 `detect(imagePath)` → `[{face, leftEye, rightEye, outerLips, innerLips}]`
   - 用 `VNDetectFaceLandmarksRequest` 检测人脸 landmark（5 类：face contour / leftEye / rightEye / outerLips / innerLips）
   - **关键**：Vision `normalizedPoints` 是**左下角原点 + Y 向上**；Dart 期望**左上角 + Y 向下**。Swift 插件里 `y = (1.0 - ny) * height` 翻 Y
   - `VNImageRequestHandler(cgImage:)` 处理，`VNDetectFaceLandmarksRequest` 异步回调；结果用 `observations.map` 转成 `[x, y]` dict 数组
   - 失败用 `FlutterError(code, message, details)` 抛，message 带 localizedDescription

2. **`ios/Runner/AppDelegate.swift`**
   - `didInitializeImplicitFlutterEngine` 钩子里注册插件
   - **关键**：直接传 `engineBridge.pluginRegistry` 编译报 `'any FlutterPluginRegistry' does not conform to 'FlutterPluginRegistrar'`——`register(with:)` 需要的是 `FlutterPluginRegistrar` 不是 registry
   - 修法：先用 `registrar(forPlugin: "FaceDetectionPlugin")` 从 registry 拿 registrar，再传给 `register(with:)`（Flutter 3.29+ API）
   - try-style if-let 兜底（拿不到 registrar 不 crash）

3. **`ios/Runner.xcodeproj/project.pbxproj`**
   - 新增 `FaceDetectionPlugin.swift` 到 `PBXFileReference` + `PBXBuildFile` + `Runner` group child + `Sources` build phase（用 Ruby `xcodeproj` gem 改的）
   - **踩坑**：`path = Runner/FaceDetectionPlugin.swift` → `path = FaceDetectionPlugin.swift`。带 `Runner/` 前缀时 `flutter build` 报 `Build input file cannot be found: '...ios/Runner/Runner/FaceDetectionPlugin.swift'`。原因：Runner 是 group name，Xcode 默认 `sourceTree = "<group>"` 时 path 已经是相对 group 的，不要再带 group name

4. **`lib/services/face_detection/ios_face_detector.dart`**（新文件）
   - Dart MethodChannel 包装 `easy_beauty_cam/face_detection`
   - `_convertFace(Map)` 把 Swift 返回的 `[{x, y}, ...]` 数组转成 `Offset` 列表
   - **关键映射**：
     - `leftEye` / `rightEye` 直接对应 FaceContours 字段
     - iOS Vision 是 `outerLips` / `innerLips`（上下唇外/内轮廓），ML Kit 是 `lipUpper` / `lipLower`（上下唇一段）。这里把 `outerLips` → `lipUpper`、`innerLips` → `lipLower` 凑合填，FaceMaskBuilder 用 OR 逻辑把两个 polygon 都从 mask 挖空，视觉上多一圈黑看不出来
     - 后续要更精细区分可加 `lipOuter` / `lipInner` 字段到 `FaceContours`

5. **`lib/services/face_detection_service.dart`**
   - 平台路由：iOS → `IOSFaceDetector().detect`，**Android → 空 stub**（`return const []`，触发「未检测到人脸」降级 UI）
   - 删了 ML Kit 的 `_mlKitDetector` / `_convert` / `_mlKitDetectorInstance` 静态字段 / `dispose()` 里的 `close()` 调用
   - **Android 端人脸检测是 TODO**：之前 ML Kit 的 iOS pod 把整个 iOS 编译都卡住了，所以一刀切删 ML Kit。Android 端需要时建议直接走平台通道调 Android 原生 `FaceDetector` API（API 14+），或找一个纯 Android 库（不发 iOS pod 的）。空 stub 期间 Android 用户拍完照编辑时 BeautySlider 会一直显示「未检测到人脸」橙色提示

6. **`pubspec.yaml`**：删 `google_mlkit_face_detection: ^0.13.2` + `google_mlkit_commons: ^0.11.0`
7. **`pubspec.lock`**：重新 `flutter pub get` 同步
8. **`ios/Podfile`**：`platform :ios, '15.5'`（之前为 ML Kit 升的）回退到 `platform :ios, '13.0'`（Vision 框架最低要求），`post_install` 里强制设 IPHONEOS_DEPLOYMENT_TARGET='15.5' 的代码也去掉
9. **`ios/Podfile.lock`**：删掉重生（不再含 ML Kit pod）

### 验证

- `flutter build ios --simulator --no-codesign` ✓ Built build/ios/iphonesimulator/Runner.app
- `grep -iE "arm64|MLKit|missing required architecture|do not support"` 在 build log 里**完全**没有相关警告
- `flutter test` 135/135 PASS（之前 132 + 〇九新加的 3 个 hint 测试全部还在跑，没回归）
- iOS 13.0 是 Vision 框架最低要求；本项目 iOS 设备部署目标现在跟着 13.0 走

### 一并修改的关系

- 〇九「人脸识别美颜」用 ML Kit 是**临时方案**：能跑通但 iOS sim 不行（ML Kit pod 不发 arm64 sim 切片）。本节是它的 iOS 端替代，**Android 端顺带也降级**（用户接受「Android 用户暂时用不了人脸识别美颜」的妥协，因为修 ML Kit 的 iOS pod 才是更痛的问题）

### 踩坑清单

- **Flutter 3.29+ `FlutterPluginRegistry.registrar(forPlugin:)`**：新 API 强制要求拿 registrar 而不是 registry。直接传 registry 编译报「does not conform to expected type 'FlutterPluginRegistrar'」
- **Xcode pbxproj `path` 字段**：当 `sourceTree = "<group>"` 时 path 是相对 group 的，不要带 group name 前缀。带了就 `Build input file cannot be found`
- **Vision normalizedPoints 坐标方向**：左下角原点 + Y 向上，必须 `1.0 - ny` 翻转 Y 才能给 Dart 用
- **ML Kit pod 不发 arm64 sim**：Google 多年公开 issue，没有官方修复。如果非要用 ML Kit，绕路是本地 build ML Kit 的 framework 或用 `EXCLUDED_ARCHS[sdk=iphonesimulator*] = arm64` 强行跑（但 Apple Silicon sim 不行）
- **iOS deployment target**：ML Kit 15.5、Vision 13.0、photo_manager 12.0。取最高的（ML Kit 15.5）就是上限，删 ML Kit 后回退到 Vision 13.0

---

<a id="appbar-and-vision-exif-bugfix-2026-06-24"></a>
### 〇十一 相机 AppBar 布局 + Vision EXIF orientation bugfix (2026-06-24)

〇十 之后真机回归发现 7 个问题，一次性修于 1 commit：
...
### 美颜 3 个问题（根因都是同一个：Vision 没处理 EXIF orientation）

1. **磨皮拉动滑杆没变化**
2. **瘦脸拉动滑杆没变化**（**已知 stub**：slim 是 MEMO 〇九 #7 明确标注"故意不实现"per spec，不是 bug）
3. **美白拉动滑杆只在照片右上角有一块地方变白，不是脸部区域变白**

**根因**：
- `image` package 4.9.1 的 `getImageFromJpeg`（`_jpeg_quantize_io.dart` line 213-265）会**烘焙 EXIF 旋转到像素**：EXIF = 6 (`.right`) 时把 raw landscape (4032×3024) 旋转成 portrait (3024×4032)，`image.width/height` 是**显示**尺寸
- 但 Swift 插件之前直接把 `cgImage` 给 `VNDetectFaceLandmarksRequest`，没传 `orientation` 参数，Vision 返回的 landmark 坐标是 **raw landscape 像素空间**（e.g. 脸中心 2000, 1500）
- 把 landscape 坐标当成 portrait 坐标用在 3024×4032 的 mask 上 → 脸位置跑到右上角
- 磨皮 6% 混合在错位置小区域几乎看不见；美白 +9 亮度在右上小区域就明显 → 用户看到"美白在右上角，磨皮无效果"

**修法** (`ios/Runner/FaceDetectionPlugin.swift`)：
- 读 `image.imageOrientation`，映射到 `CGImagePropertyOrientation`（`UIImage.Orientation` ≠ `CGImagePropertyOrientation`，需要 switch 转）
- 给 `VNImageRequestHandler(cgImage:orientation:options:)` 传 orientation 参数，让 Vision 内部把图旋转到显示方向再处理
- 用 `displaySize(cgImage, orientation)` 算显示宽高：`.right/.left/.rightMirrored/.leftMirrored` 时 displayW=rawH, displayH=rawW；其余相等
- `polygon()` 用 display 尺寸做反归一化，返回的 landmark 坐标就是 display 空间，跟 Dart 端 `img.decodeImage` 出来的 `image.width/height` 匹配
- 注释里记录了 raw `cgImage` 不带 EXIF、`image` package 会烘焙 EXIF 的不一致——后人接坑时看注释就知道为什么必须传 orientation

### 验证

- 135 → 139 个测试（+4：`bordered: false` / `bordered: true` 默认 / `iconOpacity` alpha / `CameraSwitchButton` 走 0.75）
- `flutter test` 139/139 PASS
- `flutter build ios --simulator --no-codesign` ✓ Built build/ios/iphonesimulator/Runner.app，5.3s 增量编译（只 FaceDetectionPlugin.swift 改了 71 行）
- 零 arm64 / MLKit 警告

### 踩坑清单

- **iOS Vision 必须传 `CGImagePropertyOrientation`**：默认 `.up`，但 iOS 相机拍出来的 JPEG EXIF 通常是 `.right` (6)——不传的话 Vision 处理的是 raw sensor 像素
- **`UIImage.Orientation` ≠ `CGImagePropertyOrientation`**：枚举值一一对应但类型不同（一个来自 UIKit，一个来自 ImageIO / CoreGraphics），需要 switch 转
- **EXIF orientation 5-8 会交换 width/height**：`.right` (6) / `.left` (8) / `.upMirrored` (5) / `.downMirrored` (7) 都是横屏 orientation
- **`image` package 4.x decodeJpg 默认烘焙 EXIF**：跟 Flutter 渲染层一致（都按 EXIF 转），所以 mask 必须在 display 空间
- **Color `.a` 在新 Flutter 是 double ∈ [0, 1]**（不是 0~255）：测试断言要 `closeTo(0.75, 0.01)` 不是 `closeTo(0.75 * 255, 1)`
